...
| タスク | 優先度 | 概要 |
|--------|--------|------|
| ~~オンボーディング強化~~ | ~~🔴 高~~ | **[実装・polish完了 2026-04-30]** js/onboarding.js 新規作成。6スライドカルーセル、Firebaseの実テーマデータ動的表示、スワップアニメーション、SVG接続線。初回フロー順序保証（オンボーディング→ニックネーム→β版モーダル）。ヘルプシートUI改善（ボタン色・矢印・上部パディング）。main にマージ済み |
| ~~読み込みタイムアウト対応~~ | ~~🟡 中~~ | **[実装完了 2026-04-30]** 8秒でソフト警告、15秒で対処法（①更新 ②再起動）を表示。正常完了時は自動キャンセル。main にマージ済み |
