...
| タスク | 優先度 | 概要 |
|--------|--------|------|
| ~~オンボーディング強化~~ | ~~🔴 高~~ | **[実装完了 2026-04-23]** `js/onboarding.js` 新規作成。6スライドカルーセル、Firebaseの実テーマデータ動的表示、スワップアニメーション、SVG接続線。`localStorage 'rankq_onboarded'` で管理。`showOnboarding(onComplete)` / `hideOnboarding()` / `resetOnboarding()`（開発用）。ヘルプシートに「ゲームの流れをもう一度見る」ボタン追加。betaにデプロイ済み |
