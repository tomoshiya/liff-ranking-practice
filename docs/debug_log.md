...
### [NEW] オンボーディング モックアップ作成（2026-04-22）
- **背景**: βテスト中のユーザーフィードバックから「初めての人がゲームを理解するまでに時間がかかる」という課題が浮上。初回起動時のオンボーディング強化を検討開始
- **方針決定**:
  - βモーダルを閉じた後に自動表示（`rankq_onboarded` キーで管理）
  - ニックネーム入力モーダルの前に表示
  - カルーセル形式（スワイプ可能）、スキップなし、戻るボタンあり
  - 最終スライドのCTAで `openNameModal()` を呼び出す
  - 「?」ヘルプ（既存4スライド）とは別コンテンツ
- **スライド構成（6枚）**:
  1. コンセプト：テーマ例吹き出し＋5枚ランクカード入れ替えアニメ
  2. テーマを選ぶ（STEP 1）：実際の `theme-card` 3行オートスクロール
  3. TOP5を書く（STEP 2）：実際の `rank-item` ＋タイピング＋スワイプアニメ
  4. 予想する（STEP 3）：実際の `rank-item` ＋スワイプ＋吹き出し
  5. 結果発表（STEP 4）：実際のスコアボード・`person-tab`・詳細UI
  6. さあ始めよう：ふたり/みんなモード紹介＋シーンタグ＋CTA
- **作成したワイヤーフレーム**: v1〜v4（`docs/wireframe_onboarding_v1〜v4.html`）
- **v4時点の改善事項**:
  - `firebase_items.json` の実際のテーマテキスト（改行含む）を使用
  - `firebase_packs.json` の正確なパックカラーに修正
  - フッターを常に白で統一（白/グレー交互問題を解消）
  - 全スライドに1〜2行のステップ説明文を追加
  - テーマ・ランク内容を「居酒屋のおつまみ」で全スライド統一
  - `user-select: none` でテキスト選択をロック
- **ステータス**: モックv4まで完成。翌日実装フィードバックを受けてv5以降へ継続
- **関連ファイル**: `docs/wireframe_onboarding_v1〜v4.html`（実装前モック）

### [NEW] オンボーディング 実装完了（2026-04-23）
- **実装内容**:
  - `js/onboarding.js` 新規作成（全6スライドのカルーセル型オンボーディング）
  - Slide2のテーマカードは `themes` / `packMeta` グローバル変数（Firebase実データ）を動的生成
  - Slide3/4スワイプアニメーション: タイマー残留バグ修正（`clearS3Timers`をrunS3冒頭で呼び出し）
  - バッジ番号をアニメーション開始前に視覚位置へ更新、完了後にDOM位置へ復元する方式
  - `localStorage 'rankq_onboarded'` で表示済み管理
  - `resetOnboarding()` でコンソールから何度でも再テスト可能
  - ヘルプシート下部に「ゲームの流れをもう一度見る」ボタンを追加
- **app.js 変更**: 初回起動時に `showOnboarding()` 呼び出し → 完了コールバックで `openFirstTimeNameModal()`
- **ui.js 変更**: `getScoreLabel` にJSDocでスコア・線スタイル仕様を一元記載、`getScoreLineStyle(diff)` 追加
- **スコア視覚仕様の確定**（ソース・オブ・トゥルース: `js/ui.js`）:
  - あたり(diff=0): ◎ / おしい(diff=1): ○ / ちかい(diff=2): △ / かすり(diff=3): ▽ / はずれ: ×
  - 接続線の「色」は左カード（正しいランク）のヘッダー色を基準とする
  - 接続線の「太さ・破線」はdiffで決定（かすり以上は破線 `stroke-dasharray="6,4"`）
- **デプロイ**: betaブランチにプッシュ済み。mainへはオンボーディング直前コミット（`5d3fa68`）までマージ
- **関連ファイル**: `js/onboarding.js`, `js/app.js`, `js/ui.js`, `index.html`, `docs/wireframe_onboarding_v7.html`

### [NEW] オンボーディングモック v8 作成（2026-04-24）
- **変更内容**:
  - Slide 1: ポディウムバーを縮小し、バー下部の暗いエリアに「プレイ人数」（ふたりで/みんなで）と「利用シーン」タグを追加
  - Slide 6: プレイ人数・利用シーンを削除し、3ステップの「はじめ方」フロー（ニックネーム→遊び方・部屋番号→スタート）に変更
- **結果**: ユーザーがv7の方が良いと判断し、v8は不採用。v7を引き続き正式版とする
- **関連ファイル**: `docs/wireframe_onboarding_v8.html`（参考用として保存）

### [FIX] オンボーディングモック v7 Slide2アニメーションバグ（2026-04-24）
- **現象**: LOVE行（4行目）のカードが不足して見える / スクロール速度が他行より著しく速い
- **原因1（アニメーション）**: `mRight2` が `translateX(-40%)→(+10%)` という範囲で動作しており、+10%でコンテンツが右にはみ出し左端に空白が発生
- **原因2（カード枚数）**: LOVE行のみ10枚（他行6枚）で、コンテンツ幅が2540px→4240pxになりアニメーション速度が約3倍速くなっていた
- **修正**: `mLeft2`/`mRight2` を `0%↔-50%` の正常範囲に修正。LOVE行カードを6枚に統一
- **関連ファイル**: `docs/wireframe_onboarding_v7.html`

### [FIX] オンボーディング各種修正（2026-04-30）
- **Slide5 SVGライン モバイルズレ修正**: `_startS5` の描画タイミングを 120ms → 420ms に変更。スライドトランジション（360ms）完了前に `getBoundingClientRect()` を呼び出していたため座標がズレていた
- **Slide5 説明文フォントサイズ統一**: インライン `font-size:10px` を `.ob-desc` クラスに変更し Slide3/4 と同一サイズに統一
- **Slide6 タイトル変更**: 「まずニックネームを設定しよう」→「ニックネームを設定してスタート！」
- **ヘルプシート「ゲームの流れをもう一度見る」ボタン**: 「さあ、はじめよう！」と同じ紫グラデーションに変更（`linear-gradient(135deg,#7C4DFF,#5c35cc)`）
- **ヘルプシート ナビゲーション矢印**: `font-size: 13px → 20px`、`padding: 6px 10px → 10px 16px` に拡大
- **ヘルプシート 上部パディング**: `.bottomsheet--help .bottomsheet__title { padding-top: 36px; }` 追加。×ボタン下に余白を確保
- **ニックネームモーダル**: 入力欄とボタンの間に「※あとから変更可能です」テキスト追加
- **初回フロー表示順修正**: `_pendingBetaCheck` フラグを導入。オンボーディング→ニックネーム入力→β版モーダルの順を保証。従来はβ版モーダルがニックネームモーダルより先に重なる問題があった
- **関連ファイル**: `js/onboarding.js`, `js/app.js`, `js/ui.js`, `index.html`, `css/beta.css`

### [NEW] 読み込みタイムアウトメッセージ実装（2026-04-30）
- **背景**: アプリ起動後に読み込み中のまま画面が切り替わらない事象が散見された
- **実装**: 2段階タイムアウトを `initializeLiff()` 内で実装
  - 8秒後: 「時間がかかっています...」のソフト警告を表示
  - 15秒後: ソフト警告を非表示にし、具体的な対処法（①「︙」→更新 / ②「✕」→再起動）を表示
- **クリア処理**: `hideLoading()` 内で `window._loadingSlowTimer` / `window._loadingTimeoutTimer` をキャンセル。正常読み込み完了時はメッセージが表示されない
- **関連ファイル**: `index.html`, `js/app.js`, `js/ui.js`
