# 🎮 RankQuest - プロジェクトマスタードキュメント（α版アーカイブ）

**最終更新**: 2026年2月2日（α版完了時点でアーカイブ化）
**アーカイブ化**: 2026年3月4日（β版移行に伴い封印）
**プロジェクト状態**: 📦 **α版アーカイブ** - このファイルは読み取り専用です
...
>
> ### このファイルの位置づけ
> このファイルは **α版（〜2026年2月）までの開発記録アーカイブ** です。
> **β版以降の開発には使用しません。更新もしません。**
>
> ### 今後の開発で参照すべきファイル
>
> | ファイル | 役割 | 更新 |
> |---|---|---|
> | **`_plan.md`** | 現在のTODO・環境情報・β版ロードマップ | ✅ 毎セッション更新 |
> | **`PROJECT_MASTER.md`** | α版までの技術決定・セーブポイントの記録 | 🔒 更新しない |
> | **`firebase-security-rules-simple.json`** | Firebaseルールの管理ファイル | ✅ 変更時に更新 |
>
> ### 新しいAIセッションを始めるときは
> 1. **まず `_plan.md` を読む**（現状・TODO・環境情報がすべて揃っている）
> 2. α版以前の経緯・技術的な決定の理由を知りたい場合のみ、このファイルを参照する
>
> ### なぜ2つのファイルに分かれているか
> - α版まで: `PROJECT_MASTER.md` を唯一のドキュメントとして運用していた
> - β版移行（2026年3月〜）: `_plan.md` を新たに作成し、こちらを主ドキュメントに切り替えた
> - 切り替えた理由: PROJECT_MASTER.mdが3000行超になり、AIのコンテキスト読み込みに不向きになったため
...
- **日時**: 2026年2月2日
- **状態**: 
  - ✅ 簡易モニタリング実装（trackEvent関数）
  - ✅ RankQuestへのリブランディング完了
  - ✅ プライバシーポリシー・利用規約作成
  - ✅ Firebase Security Rules 最適化
  - ✅ テスター運用ドキュメント整備
  - ✅ α版テスト準備完了
- **主な変更**:
  1. Analytics: ユーザー登録、ログイン、ペア接続、ゲーム開始を記録
  2. ブランディング: 「ふたりのらんきんぐ」→「RankQuest (α版)」
  3. ドキュメント: FEEDBACK_FORM_TEMPLATE.md追加
  4. Security: firebase-security-rules-simple.json作成
- **戻す方法**: `git reset --hard 2afcae3`

### #96: ゲームフロー改善＋シンプル招待機能✨
- **コミットID**: `34e2715`
- **コミットメッセージ**: "fix: prevent duplicate notifications for pair operations"
- **日時**: 2026年1月31日
- **状態**: 
  - ✅ ゲームフロー改善（モード選択を待機室後に変更）
  - ✅ 「もう一度遊ぶ」でモード選択に戻る
  - ✅ ロック機能のバグ修正（unlock関数追加）
  - ✅ シンプルLINE招待機能（ペア設定URL、待機室URL）
  - ✅ パートナー名プレビュー機能
  - ✅ リアルタイムペア更新通知
  - ⚠️ 既知の軽微な問題：ペア設定・解除時に操作側でポップアップが2回表示される
- **主な変更**:
  1. ゲームフロー: ゲスト参加後にモード・お題を決定
  2. ロック解除: `unlockGameInput()`、`unlockGuessInput()`追加
  3. 招待機能: URL招待（LINE共有→URLコピーに変更）
  4. UI改善: パートナー名プレビュー、リアルタイム通知
- **戻す方法**: `git reset --hard 34e2715`

### #85: Phase 7完了（セーフポイント・セキュリティ実装済み）
- **コミットID**: `87ec262`
- **コミットメッセージ**: （記録なし）
- **日時**: 2026年1月26日頃
- **状態**: 
  - ✅ Security Rules更新完了
  - ✅ 匿名認証実装済み
  - ✅ 予想画面＋回答画面のSortableJS
  - ✅ 実機テスト成功
  - ✅ Phase 8前の安定版
- **戻す方法**: `git reset --hard 87ec262`

### #83: 回答入力スワイプバグ修正
- **コミットID**: `397dcf7`
- **コミットメッセージ**: "fix: game input screen bugs (placeholder, order reset, Enter key navigation)"
- **日時**: 2026年1月28日
- **状態**: 
  - ✅ 回答入力画面のスワイプ完全動作
  - ✅ プレースホルダーバグ修正
  - ✅ 2ゲーム目の順位保持バグ修正
  - ✅ Enterキー挙動修正（DOM順序ベース）
- **修正内容**:
  1. プレースホルダーがスワイプで移動 → 順位に連動するよう修正
  2. 2ゲーム目で順位保持 → DOM順序をリセット
  3. Enterキーがスワイプ前のIDを参照 → DOM順序ベースに変更
- **戻す方法**: `git reset --hard 397dcf7`

### #82: 回答入力スワイプ対応（バグあり・非推奨）
- **コミットID**: `06e25f8`
- **コミットメッセージ**: "feat: add swipe functionality to game input screen with SortableJS"
- **日時**: 2026年1月28日
- **既知のバグ**:
  - ⚠️ プレースホルダーがスワイプで移動
  - ⚠️ 2ゲーム目で順位が保持される
  - ⚠️ Enterキーの挙動がおかしい
- **戻す方法**: `git reset --hard 06e25f8`

### #81: Phase 7 完了（スワイプ対応前・安全）
- **コミットID**: `459032c`
- **コミットメッセージ**: "docs: Phase 7 Security Implementation Complete"
- **日時**: 2026年1月26日
- **状態**: 
  - ✅ Security Rules更新完了
  - ✅ 匿名認証実装済み
  - ✅ 予想画面のSortableJSのみ
  - ✅ 実機テスト成功
- **特徴**: 回答入力スワイプなし、最も安定したバージョン
- **戻す方法**: `git reset --hard 459032c`

### #80以前: MVP開発フェーズ
- Security Rules実装
- Firebase Authentication SDK追加
- 匿名認証実装

---

## 🔄 Phase 8: シンプルLINE招待機能（簡易版完了）

### 実装状況
**Phase 8（簡易版）完了** - 複雑な複数ペア機能は見送り、シンプルな招待機能を実装

### 実装済み機能
1. ✅ **ゲームフロー改善**
   - ゲスト参加後にモード・お題を決定（待機室では決めない）
   - 「もう一度遊ぶ」でモード選択画面に戻る
   - 待機室のお題表示を削除

2. ✅ **ロック機能のバグ修正**
   - `unlockGameInput()`、`unlockGuessInput()`追加
   - 「もう一度遊ぶ」後のロック状態を正しく解除

3. ✅ **シンプルLINE招待機能**
   - **待機室URL招待**: ホストが待機室URLをコピーして招待
   - **ペア設定URL招待**: ペアコードURLで簡単ペア設定
   - URLパラメータによる自動遷移
   - 招待者判定（ホスト/パートナー/非パートナー/既ペア済み）
   - パートナー名プレビュー（ペアコード入力時）
...
- ❌ 複数ペア管理（最大3名）
- ❌ パートナー選択画面
- ❌ `friendships` / `invitations` ノード
- ❌ トークン方式の招待

### 既知の軽微な問題
- ⚠️ ペア設定・解除時、操作側でポップアップが2回表示される
  - 影響: 軽微（操作に支障なし）
  - 対応: 予算の都合上、いったんステイ

### 技術的詳細
- **データ構造**: 既存の`users`、`pairCodes`、`gameSessions`を活用
- **URL招待**: `?session=xxx` / `?pairCode=xxx`
- **招待判定**: Firebase検索でユーザー情報を取得
- **リスナー**: `setupPairListener()`でリアルタイム監視
...
**期間**: 2026年2月3日〜2月23日（3週間）  
**目的**: 基本機能の動作確認、致命的なバグの発見、初期フィードバック収集  
**参加者**: 10-40名（テスター登録）  
**チャネル状態**: 開発中（テスターのみアクセス可能）
...
#### α版の実施内容
```
Week 1（2/3-9）: クローズドβ Phase 1
  □ パートナーとテストプレイ
  □ 友人3-5組（6-10人）をテスター登録
  □ 基本動作確認
  □ バグ修正

Week 2（2/10-16）: クローズドβ Phase 2
  □ テスター10-20人に拡大
  □ フィードバック収集（Googleフォーム）
  □ UI/UX改善
  □ バグ修正
...
**期間**: 2026年2月24日〜4月末（2ヶ月）  
**目的**: 実ユーザー獲得、本格フィードバック収集、機能拡張、市場検証  
**参加者**: 無制限（全LINEユーザー）  
**チャネル状態**: 公開済み（不可逆）
...
#### β版で追加する機能（優先順位順）
```
【Phase 9: コード品質向上】（β版公開前）
1. □ console.log削除
2. □ コード分割（CSS/JS分離）
3. □ エラーメッセージ日本語化
4. □ チャネルアイコン設定

【Phase 10: モニタリング強化】（β版初期）
5. □ Google Analytics 4 導入
6. □ カスタムイベント追加
   - テーマ選択
   - スコア分布
   - セッション時間
...
【Phase 11: セキュリティ強化】（β版初期）
8. □ Security Rules 監査
9. □ レート制限検討
10. □ 不正アクセス監視

【Phase 12: UX改善】（フィードバック反映）
11. □ チュートリアル画面追加
12. □ ヘルプ/FAQ画面
13. □ エラー時の誘導改善
14. □ ローディング表示改善
...
**期間**: 2026年5月以降  
**目的**: マネタイズ開始、グローバル展開、事業化  
**詳細**: BUSINESS_STRATEGY.md 参照

---

## 🔖 SortableJS統合完了版セーフポイント（旧）

> **⚠️ 参考：MVP完成版（予想画面のみSortableJS）**
...
- **コミット #99** (`275dced`): LAUNCH_GUIDE.md作成
...
- デバッグ用メッセージ
- F12キーで開発者ツールから確認可能
- セキュリティリスク・パフォーマンス低下の原因
- β版公開前に削除推奨

【GitHubセキュリティ】
- リポジトリ: Public（一般公開）
- リスク: コードコピー、Firebase設定露出
- 対策: Security Rulesで保護済み
- 判断: α版・β版段階ではPublicのままで問題なし
...
【コード分割】
- 現状: index.html 4,600行超
- 課題: 1ファイルが巨大
- 対策: α版完了後、β版準備中にリファクタリング

【モニタリング】
- α版: Firebaseベースの簡易記録
- β版: Google Analytics 4 導入
- KPI: ユーザー数、プレイ数、リテンション等
```
...
### ✅ Phase 8（簡易版）完了: ゲームフロー改善＋シンプル招待機能
- **Phase 8からの戻し**: `git reset --hard 87ec262`でセーフポイント#85に戻す
- **コミット #89** (`b4ab76c`): ゲームフロー改善（モード選択タイミング変更）
- **コミット #90** (`4e9bd29`): ロック機能バグ修正（unlock関数追加）
- **コミット #91** (`cfd8f68`): シンプル招待機能実装（URL招待）
- **コミット #92** (`453a057`): 招待機能改善（判定ロジック）
- **コミット #93** (`a1cb916`): 招待UI/UX改善（ボタン統一、名前プレビュー）
- **コミット #94** (`4be056c`): 最終UI調整＋リアルタイム更新
- **コミット #95** (`dcc6347`): パートナー名表示調整、ポップアップ修正
- **コミット #96** (`34e2715`): ポップアップ2回表示の修正試行（効果なし、ステイ）
- **状態**: デプロイ済み、実機テスト完了
...
### 軽微な問題（2026年1月31日）
1. **ペア設定・解除時のポップアップ2回表示**
   - **現象**: 操作した側で「✅ ペア設定完了！」→「✨ ペアが成立しました！」と2回表示される
   - **影響**: 軽微（操作に支障なし）
   - **状況**: 修正を試行したが効果なし、いったんステイ
   - **対応時期**: 予算リセット後に対応を検討
...
### 2026年1月31日（Phase 8簡易版: ゲームフロー改善＋シンプル招待機能）
- ✅ **Phase 8からの戻し作業**
  - コミット`87ec262`（セーフポイント#85）まで戻し
  - 複雑な複数ペア機能を見送り、シンプルな実装を選択
  - 予算の94%消費状況を考慮した判断

- ✅ **ゲームフローの大幅改善**
  - **モード選択タイミングの変更**: 待機室開設前→ゲスト参加後
  - **待機室のシンプル化**: お題表示を削除
  - **「もう一度遊ぶ」の改善**: モード選択画面に戻る（ホストのみ）
  - **新関数**: `createWaitingSession()`, `startGameWithTheme()`

- ✅ **ロック機能のバグ修正**
  - **問題**: 「もう一度遊ぶ」後に回答入力がロックされたまま
  - **原因**: Firebase上の`rankings`等がクリアされていない＋DOM/SortableJS状態が残存
  - **修正1**: `startGameWithTheme()`で`rankings: null`等を明示的にクリア
  - **修正2**: `unlockGameInput()`、`unlockGuessInput()`関数を新規作成
  - **修正3**: `showGameInputScreen()`等で未送信時に`unlock`関数を呼び出し

- ✅ **シンプルLINE招待機能の実装**
  - **待機室URL招待**: `?session=xxx`でゲストが直接待機室に参加
  - **ペア設定URL招待**: `?pairCode=xxx`でペアコードを自動入力
  - **URL処理**: `checkUrlParameters()`、`handleSessionInvite()`、`handlePairCodeInvite()`
  - **招待判定**: ホスト/パートナー/非パートナー/既ペア済みを区別

- ✅ **UI/UX改善（複数ラウンド）**
  - **待機室ボタン**: 「📱 LINEで招待を送る」→「📋 待機室のURLをコピー」（青色）
  - **ペア設定ボタン**: 「📋 コードをコピー」「📱 LINEで送る」→「📋 ペア設定用のURLをコピー」（紫色）
  - **パートナー名プレビュー**: ペアコード入力時に「〇〇さんのペアコード」と表示
  - **自動遷移**: 自分の招待URL/既ペア済みの招待URLは自動でHOMEへ
  - **セクション非表示**: 招待URLから入った場合は「あなたのペアコード」を非表示

- ✅ **リアルタイムペア更新機能**
  - **リスナー追加**: `setupPairListener()`でペア状態を監視
  - **通知**: ペア成立時/解除時にポップアップ表示（ゲーム中以外）
  - **画面リフレッシュ**: HOME画面またはペア設定画面を自動更新

- ✅ **ポップアップ2回表示の修正試行**
  - **問題**: ペア設定・解除時、操作側でポップアップが2回表示される
  - **試行**: `skipNextPairNotification`フラグで操作直後の通知をスキップ
  - **結果**: 効果なし、ステイと判断
  - **影響**: 軽微（操作に支障なし）

- ✅ **Git作業履歴**
  - コミット`b4ab76c`: ゲームフロー改善
  - コミット`4e9bd29`: ロック機能バグ修正
  - コミット`cfd8f68`: シンプル招待機能実装
  - コミット`453a057`: 招待機能改善
  - コミット`a1cb916`: 招待UI/UX改善
  - コミット`4be056c`: 最終UI調整＋リアルタイム更新
  - コミット`dcc6347`: パートナー名表示調整、ポップアップ修正
  - コミット`34e2715`: ポップアップ2回表示の修正試行（効果なし）
...
**🎯 次回の推奨順序：**
1. **パートナーとテストプレイ**（最優先・動作確認）
2. **友人3-5組にテスター依頼**（LINE UserID取得→登録）
3. **フィードバック収集**（Googleフォーム作成）
4. **バグ修正**（必要に応じて）
5. **α版継続**（10-20人に拡大）
...
**⚠️ 既知の軽微な問題：**
- ペア設定・解除時のポップアップ2回表示（操作側のみ、影響軽微）
- console.log が112箇所残っている（β版公開前に削除予定）
